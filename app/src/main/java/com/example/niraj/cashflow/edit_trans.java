...
public class edit_trans extends Activity {
    String str,prev_string="";
    private int year,month,day,hour,min;
    String yr="",mnt="",dy="",hr="",mn="";
    EditText description,amount;
    String tdate,ttime,taccount,tcategory,ttype,tdescription,tamount;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tamount = sharedPreferences.getString("tamount", "");
        tdescription=sharedPreferences.getString("tdescription","");
        description.setText(tdescription);
        amount.setText(tamount);
        taccount = sharedPreferences.getString("taccount", "");
        tcategory = sharedPreferences.getString("tcategory", "");
        for(int i=0;i<arrayList.size();i++)
        {
            String s = arrayList.get(i).toString();
            if (s.equalsIgnoreCase(tcategory))
            {
                sp.setSelection(i);
            }
        }
        ...
        {
            String s = arrayList1.get(i).toString();
            if (s.equalsIgnoreCase(taccount))
            {
                sp1.setSelection(i);
            }
        }
        ttype = sharedPreferences.getString("ttype", "");
        if(ttype.equalsIgnoreCase("income"))
            aSwitch.setChecked(false);
        else
            aSwitch.setChecked(true);

        tdate = sharedPreferences.getString("tdate", "");
        ttime = sharedPreferences.getString("ttime", "");
        changetime.setText(ttime);
        changeDate.setText(tdate);
    }
    public void done_trans(View view)
    {
        db.execSQL("DELETE from transactions where amount =" + tamount+" and description = '"+tdescription+"' and lineartime='"+ttime+"'");
        try {
            ...
            if (des != null) {
                ...
                time = changetime.getText().toString();
                date = changeDate.getText().toString();
